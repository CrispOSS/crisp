...
case class ElevatorRequest(floor: Int)
case class ElevatorArrival(floor: Int)
...
class Elevator() extends Actor {
	...
	def receive = {
		case ElevatorRequest(floor: Int) => {
			if (!moving.get()) {
	...
	def announce() = {
		...
		controller ! ElevatorArrival(floor.get())
	}
...
class Passenger() extends Actor {
	...
	def receive = {
		case ElevatorArrival(floor: Int) => {
			...
			if (onElevator.get()) {
				if (to.get() == floor) {
					...
					sender ! ElevatorRequest(from.get())
				}
			} else {
				if (from.get() == floor) {
...
class Controller(val elevators: ActorRef, val passengers: ActorRef) extends Actor {
	...
	def receive = {
		case ElevatorArrival(floor: Int) => {
			...
			passengers ! ElevatorArrival(floor)
		}
		case ElevatorRequest(floor: Int) => {
			//println("request: " + floor)
			elevators ! ElevatorRequest(floor)
		}
	}
...
object Building {
	...
	def start(floors: Int) = {
		...
		controller ! ElevatorRequest(r.nextInt(topFloor) + 1)
